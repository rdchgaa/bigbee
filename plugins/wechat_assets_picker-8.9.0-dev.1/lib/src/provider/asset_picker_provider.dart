...
abstract class AssetPickerProvider<Asset, Path> extends ChangeNotifier {
  ...
  final bool showOriginalBtn;
  final bool showMergeSend;
  ...
  int _selectType = 2; //1：图片 ，2：全部，3：视频
  ...
  //图片 imageList
  List<Asset> get currentImageAssets => _currentImageAssets;
  List<Asset> _currentImageAssets = <Asset>[];

  set currentImageAssets(List<Asset> value) {
    if (value == _currentImageAssets) {
      return;
    }
    _currentImageAssets = value.toList();
    notifyListeners();
  }

  //视频 videoList
  List<Asset> get currentVideoAssets => _currentVideoAssets;
  List<Asset> _currentVideoAssets = <Asset>[];

  set currentVideoAssets(List<Asset> value) {
    if (value == _currentVideoAssets) {
      return;
    }
    _currentVideoAssets = value.toList();
    notifyListeners();
  }
...
class DefaultAssetPickerProvider
  ...
  Future<void> getAssetsFromPath([int? page, AssetPathEntity? path]) {
    Future<void> run() async {
      ...
      if (currentPage == 0) {
        _currentImageAssets.clear();
        _currentVideoAssets.clear();
        _currentAssets.clear();
      }
      _currentAssets.addAll(list);

      for(AssetEntity asset in list){
        if(asset.type==AssetType.image){
          _currentImageAssets.add(asset);
        }else if(asset.type==AssetType.video){
          _currentVideoAssets.add(asset);
        }
      }
