...
class TUIChatGlobalModel extends ChangeNotifier implements TIMUIKitClass {
  ...
  downloadFile() async {
    print("下载进度map - ${_messageListProgressMap}");
    if (_isDownloading || _waitingDownloadList.isEmpty) {
      return;
    }
    print("下载进度map1 - ${_messageListProgressMap}");
  ...
  clearCurrentConversation() {
    ...
    _messageListMap[currentSelectedConv] = (_messageListMap[currentSelectedConv] ?? []).sublist(0, min(10, max(((_messageListMap[currentSelectedConv] ?? []).length - 1), 0)));
  ...
  // 删除消息
  onMessageDelete(String msgID, [String? convID]) {
    final activeMessageList = _messageListMap[convID ?? currentSelectedConv];
    if (activeMessageList != null) {
      final findeIndex = activeMessageList.indexWhere((element) => element.msgID == msgID);
      if (findeIndex != -1) {
        ...
        if (findeIndex != -1) {
          final targetItem = activeMessageList[findeIndex];
          targetItem.status = MessageStatus.V2TIM_MSG_STATUS_HAS_DELETED;
          targetItem.id = DateTime.now().millisecondsSinceEpoch.toString();
          activeMessageList[findeIndex] = targetItem;
        }
      }
  ...
  // 删除消息
  onMessagesDelete(List<String> msgIDs, [String? convID]) {
    TencentImSDKPlugin.v2TIMManager.getMessageManager().deleteMessages(msgIDs: msgIDs);
  }

  onMessageModified(V2TimMessage modifiedMessage, [String? convID]) async {
    if (MessageUtils.isDeleteMessage(modifiedMessage)) {
      onMessagesDelete([modifiedMessage.msgID ?? ""], convID);
    }
  ...
  List<V2TimMessage>? getMessageList(String conversationID) {
    final allList = messageListMap[conversationID]?.reversed.toList() ?? [];
    final deletedList = allList.where((element){
      return MessageUtils.isDeleteMessage(element);
    }).toList();

    Future.delayed(Duration(milliseconds: 200)).then((_) async {
      if(deletedList.isNotEmpty){
        onMessagesDelete(deletedList.map((e) => e.msgID ?? '').toList());
      }
    });
    final list = allList.where((element) => _lifeCycle?.messageShouldMount(element) ?? true).toList();
