...
class TUIChatSeparateViewModel extends ChangeNotifier {
  ...
  V2TimMessage? get editorMessage => _editorMessage;

  set editorMessage(V2TimMessage? value) {
    if (value != null) {
      _repliedMessage = null;
    }
    _editorMessage = value;
    _notify();
  }
...
// 拼接聊天记录
  ...
  translateText(V2TimMessage message) async {
    ...
    String lc = TencentUtils.checkString(message.localCustomData) ?? "";
  ...
  convertVoiceToText(V2TimMessage message) async {
    final localCustomData = LocalCustomDataModel(isLoading: true);
    message.localCustomData = localCustomData.toString();
    globalModel.onMessageModified(message);

    final res = await TencentImSDKPlugin.v2TIMManager.getMessageManager().convertVoiceToText(msgID: message.msgID ?? "", language: "zh");
    if (res.code == 0) {
      final localCustomData1 = LocalCustomDataModel(soundText: res.data);
      message.localCustomData = localCustomData1.toString();
      globalModel.onMessageModified(message);
      // await TencentImSDKPlugin.v2TIMManager.v2TIMMessageManager.setLocalCustomData(
      //     msgID: message.msgID!, localCustomData: localCustomData1.toString());
    }
  }
  ...
  Future<V2TimMessage?> getMessageByID(String msgID) async {
    V2TimMessage? message = await tools.getExistingMessageByID(
            msgID: msgID,
            conversationType: conversationType ?? ConvType.c2c,
            conversationID: conversationID);
    return message;
  ...
  Future<V2TimValueCallback<V2TimMessage>?> sendMessage({required String convID, required ConvType convType, required String id, required V2TimMessage messageInfo}) async {
...
//   // 发送合并图片消息
//   Future<V2TimValueCallback<V2TimMessage>?> sendMixedImageMessage({
...
//           imagePath, target,
//           format: CompressFormat.jpeg, quality: (isOriginImage ? 100 : 60));
//       image = result?.path;
...
//   }
  ...
  Future<V2TimValueCallback<V2TimMessage>?> reSendFailMessage({
    ...
    await deleteMsg(message.msgID ?? "",
        id: message.id, webMessageInstance: message.messageFromWeb, isDeleteOther: false);
  ...
  deleteMsg(String msgID, {String? id, Object? webMessageInstance, bool isDeleteOther = false}) async {
    if (lifeCycle?.shouldDeleteMessage != null &&
        await lifeCycle!.shouldDeleteMessage(msgID) == false) {
      return;
    }
    if (isDeleteOther) {
      final messageList = await _messageService.findMessages(messageIDList: [msgID]) ?? [];
      if(messageList.isNotEmpty){
        V2TimMessage originMessage = messageList[0];
        originMessage.cloudCustomData = jsonEncode({"isDelete": true});
        final res = await TencentImSDKPlugin.v2TIMManager.getMessageManager().modifyMessage(message: originMessage);
      }
    }
  ...
  deleteSelectedMsg(bool isDeleteOther) async {
    ...
    if (isDeleteOther) {
      // 编辑消息（标识为删除， 对方监听到消息变更后回刷新）
      final msgList = List<V2TimMessage>.from(_multiSelectedMessageList.map((e) {
        V2TimMessage originMessage = e;
        originMessage.cloudCustomData = jsonEncode({"isDelete": true});
        return originMessage;
      }));
      for (var e in msgList) {
        final res = await TencentImSDKPlugin.v2TIMManager.getMessageManager().modifyMessage(message: e);
        // print("编辑消息 - ${res.toJson()}");
      }
    }
