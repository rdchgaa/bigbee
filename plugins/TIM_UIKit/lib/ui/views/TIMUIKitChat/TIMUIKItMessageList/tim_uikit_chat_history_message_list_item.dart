...
/// 增加点击回复消息回调【ab】24.07.13
typedef MessageItemContent1 = Widget? Function(
    V2TimMessage message,
    bool isShowJump,
    VoidCallback clearJump,
...
/// 增加chatModel【ab】24.07.13
typedef MessageItemContent2 = Widget? Function(
    V2TimMessage message,
    bool isShowJump,
    VoidCallback clearJump,
    TUIChatSeparateViewModel chatModel,
...
class MessageItemBuilder {
  ...
  /// 增加回复消息回调【ab】24.07.13
  final MessageItemContent1? textReplyMessageItemBuilder;
  ...
  final MessageItemContent2? imageMessageItemBuilder;

  /// sound message builder, returns null means using default widget.
  final MessageItemContent2? soundMessageItemBuilder;

  /// video message builder, returns null means using default widget.
  final MessageItemContent2? videoMessageItemBuilder;

  /// file message builder, returns null means using default widget.
  final MessageItemContent2? fileMessageItemBuilder;
  ...
  final MessageItemContent2? mergerMessageItemBuilder;
...
class TIMUIKitHistoryMessageListItem extends StatefulWidget {
  ...
  const TIMUIKitHistoryMessageListItem(
      ...
      this.customMessageHoverBarOnDesktop, this.messageCollectFunction})
      : super(key: key);
...
class _TIMUIKItHistoryMessageListItemState extends TIMUIKitState<TIMUIKitHistoryMessageListItem> with TickerProviderStateMixin {
  ...
  Widget _groupCallMessageBuilder(TUIChatSeparateViewModel model, TUITheme theme) {
    final text = MessageUtils.getGroupCallText(widget.message);
    if (text.isEmpty) {
      return Container();
    }
  ...
  Widget _customGroupTipMessageBuilder(TUIChatSeparateViewModel model, TUITheme theme) {
    final text = MessageUtils.getCustomGroupTipText(widget.message);
    if (text.isEmpty) {
      return Container();
    }
    return MessageUtils.wrapMessageTips(
  ...
  initTools({BuildContext? context, bool isLongMessage = false, required TUIChatSeparateViewModel model, TUITheme? theme, bool? isShowMoreSticker, TapDownDetails? details, bool? isFromWideToolTip}) {
    ...
    tooltip = SuperTooltip(
      ...
      content: TIMUIKitMessageTooltip(
        ...
        }, messageCollectFunction: widget.messageCollectFunction,
      ),
    );
  }
  ...
  Widget tuiBuild(BuildContext context, TUIKitBuildValue value) {
    ...
    final isGroupCallMsg = MessageUtils.isGroupCallingMessage(widget.message);
    final isCustomGroupTipMsg = MessageUtils.isCustomGroupTipMessage(widget.message);
    ...
    if (isGroupCallMsg) {
      return _groupCallMessageBuilder(model, theme);
    }
    // TODO: 群自定义提示消息
    if (isCustomGroupTipMsg) {
      return _customGroupTipMessageBuilder(model, theme);
    }
    ...
    return LayoutBuilder(
      builder: (context, constraints) => Container(
        ...
        child: Row(
          ...
          children: [
            if (model.isMultiSelect && widget.message.status == MessageStatus.V2TIM_MSG_STATUS_SEND_SUCC)
              Container(
                ...
                child: GestureDetector(
                  ...
                  onTap: () {
                    if (model.isMultiSelect) {
                      ...
                      if (checked) {
                        ...
                        if (message.status == MessageStatus.V2TIM_MSG_STATUS_SENDING || message.status == MessageStatus.V2TIM_MSG_STATUS_SEND_FAIL) {
                          return;
                        }
                        model.addToMultiSelectedMessageList(message);
                      }
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Container(
                        margin: widget.showAvatar ? (isSelf ? const EdgeInsets.only(right: 0) : const EdgeInsets.only(left: 8)) : null,
                        child: Column(
                          ...
                          children: [
                            if ((isSelf && isShowNickNameForSelf) || (!isSelf && isShowNickNameForOthers))
                              widget.topRowBuilder != null
                                  ...
                                  : Container(
                                      margin: const EdgeInsets.only(bottom: 4, left: 14, right: 14),
                                      child: ConstrainedBox(
                                        constraints: BoxConstraints(maxWidth: MediaQuery.of(context).size.width / 1.7),
                                        child: Text(
                                          MessageUtils.getDisplayName(message),
                                          overflow: TextOverflow.ellipsis,
                                          style: widget.themeData?.nickNameTextStyle ?? TextStyle(fontSize: 14, color: theme.weakTextColor),
                                        ),
                                      )),
                            Row(
                              crossAxisAlignment: CrossAxisAlignment.end,
