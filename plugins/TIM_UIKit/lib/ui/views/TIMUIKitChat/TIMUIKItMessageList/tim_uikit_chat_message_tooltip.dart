...
import 'package:tencent_cloud_chat_uikit/ui/widgets/tim_alert_widget.dart';
...
class TIMUIKitMessageTooltip extends StatefulWidget {
  ...
  final MessageCollectFunction? messageCollectFunction;
...
class TIMUIKitMessageTooltipState
  ...
  bool isRevocable(int timestamp, int upperTimeLimit) =>
      ((DateTime.now().millisecondsSinceEpoch / 1000).ceil() - timestamp <
          upperTimeLimit) &&
      (widget.message.isSelf ?? true) && widget.message.status  == MessageStatus.V2TIM_MSG_STATUS_SEND_SUCC;
  ...
  _buildLongPressTipItem(
    ...
    final isCallingMessage = widget.message.customElem?.data != null &&
    ...
    final shouldShowReplyAction = (!(widget.message.customElem?.data != null &&
        MessageUtils.isCallingData(widget.message.customElem!.data!)) || isCallingMessage) && widget.message.status  == MessageStatus.V2TIM_MSG_STATUS_SEND_SUCC;
    final shouldShowEditorAction = widget.message.elemType == MessageElemType.V2TIM_ELEM_TYPE_TEXT && (widget.message.isSelf ?? false) && widget.message.status  == MessageStatus.V2TIM_MSG_STATUS_SEND_SUCC;
    final shouldShowConvertAction = widget.message.elemType == MessageElemType.V2TIM_ELEM_TYPE_SOUND && widget.message.status  == MessageStatus.V2TIM_MSG_STATUS_SEND_SUCC;
    final shouldShowMultiAction = widget.message.status == MessageStatus.V2TIM_MSG_STATUS_SEND_SUCC;
    final shouldShowForwardAction = MessageUtils.isMessageCanForward(message: message);
    final shouldShowCollectAction = MessageUtils.isMessageCanCollect(message: message);
    ...
    final List<MessageToolTipItem> defaultTipsList = [
      ...
      if (shouldShowEditorAction)
        MessageToolTipItem(
            label: TIM_t(
                "编辑"),
            id: "editorMessage",
            iconImageAsset: "images/editor.png",
            onClick: () => _onTap("editorMessage", model, theme)),
      if (messageCanCopy)
        MessageToolTipItem(
            label: TIM_t("复制"),
            id: "copyMessage",
            iconImageAsset: "images/copy_message.png",
            onClick: () => _onTap("copyMessage", model, theme)),
      if (shouldShowCollectAction )
        MessageToolTipItem(
            label: TIM_getCurrentDeviceLocale().contains("zh") ? "收藏" : "Collect",
            id: "forwardMessage",
            iconImageAsset: "images/collect.png",
            onClick: () => _onTap("collectMessage", model, theme)),
      if (shouldShowForwardAction && !isVoteMessage(widget.message))
        MessageToolTipItem(
            label: TIM_t("转发"),
            id: "forwardMessage",
            iconImageAsset: "images/forward_message.png",
            onClick: () => _onTap("forwardMessage", model, theme)),
      if (shouldShowReplyAction)
        MessageToolTipItem(
            ...
            onClick: () => _onTap("replyMessage", model, theme)),
      if (shouldShowConvertAction)
        MessageToolTipItem(
            label: TIM_t("转文字"),
            id: "convertToSound",
            iconImageAsset: "images/convert.png",
            onClick: () => _onTap("convertToSound", model, theme)),
      if (shouldShowMultiAction)
        MessageToolTipItem(
            label: TIM_t("多选"),
            id: "multiSelect",
            iconImageAsset: "images/multi_message.png",
            onClick: () => _onTap("multiSelect", model, theme)),
      // MessageToolTipItem(
      //     label: TIM_t("翻译"),
      //     id: "translate",
      //     iconImageAsset: "images/translate.png",
      //     onClick: () => _onTap("translate", model, theme)),
      MessageToolTipItem(
          label: TIM_t("删除"),
          id: "delete",
          iconImageAsset: "images/delete_message.png",
          onClick: () => _onTap("delete", model, theme)),
      if (shouldShowRevokeAction)
        MessageToolTipItem(
            label: TIM_t("撤回"),
            id: "revoke",
            iconImageAsset: "images/revoke_message.png",
            onClick: () => _onTap("revoke", model, theme)),
    ];
    ...
    if (tooltipsConfig != null) {
      defaultFormattedTipsList = defaultTipsList.where((element) {
        ...
        if (type == "editorMessage") {
          return !(isDesktopScreen && widget.iSUseDefaultHoverBar);
        }
        if (type == "delete") {
  ...
  _onTap(String operation, TUIChatSeparateViewModel model, TUITheme theme) async {
    print("是否是群管理员 - ${widget.model.groupInfo.isManager}");
    final messageItem = widget.message;
    final msgID = messageItem.msgID as String;
    final isSelf = messageItem.isSelf ?? false;
    switch (operation) {
      case "open":
        if (widget.message.fileElem != null) {
      ...
      case "delete":
        showCupertinoModalPopup<String>(
          ...
          builder: (BuildContext context) {
            return CupertinoActionSheet(
              ...
              actions: [
                if (isSelf || widget.model.groupInfo.isManager == true) CupertinoActionSheetAction(
                  onPressed: () {
                    model.deleteMsg(msgID, webMessageInstance: messageItem.messageFromWeb, isDeleteOther: true);
                    Navigator.pop(
                      context,
                      "cancel",
                    );
                ...
                CupertinoActionSheetAction(
                  onPressed: () {
                    model.deleteMsg(msgID, webMessageInstance: messageItem.messageFromWeb, isDeleteOther: false);
                    Navigator.pop(
                      context,
                      "cancel",
                    );
        ...
        // final title = TIM_getCurrentDeviceLocale().contains("zh") ? "删除该条消息？" : "Delete the message?";
        // TimAlertWidget.show(context, title, theme, onPressed: () {
        //   model.deleteMsg(msgID, webMessageInstance: messageItem.messageFromWeb);
        // });
      ...
      final title = TIM_getCurrentDeviceLocale().contains("zh") ? "撤回该条消息？" : "Revoke the message?";
      TimAlertWidget.show(context, title, theme, onPressed: () {
        model.revokeMsg(
            msgID,
            !isRevocable(
      ...
      case "multiSelect":
        ...
        break;
      case "forwardMessage":
        if (!MessageUtils.isMessageCanForward(message: widget.message)) {
          CommonToast.showView(msg: TIM_t("当前消息不支持转发"));
          return;
        }
        model.addToMultiSelectedMessageList(widget.message);
        Navigator.push(
      ...
      case "replyMessage":
        ...
        break;
      case "editorMessage":
        model.editorMessage = widget.message;
        break;
      case "convertToSound":
        widget.model.convertVoiceToText(widget.message);
        break;
        case "collectMessage":
          widget.messageCollectFunction?.call([widget.message]);
      default:
        onTIMCallback(TIMCallback(
            type: TIMCallbackType.INFO,
...
extension TimI18nUtils on I18nUtils {

  bool isZh() {
    print("语言环境 - ${t}");
    return t == AppLocale.zhHant.build() || t == AppLocale.zhHans.build();
  }

}


extension TUiGroupExtensions on V2TimGroupInfo? {
  //是否可以踢人
  bool get isManager {
    final isGroupOwner =
