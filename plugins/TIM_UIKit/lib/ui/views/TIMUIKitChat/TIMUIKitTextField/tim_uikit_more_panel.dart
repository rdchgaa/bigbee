...
import 'package:tencent_cloud_chat_uikit/ui/views/TIMUIKitChat/TIMUIKitTextField/tim_call_select_member_page.dart';
...
class _MorePanelState extends TIMUIKitState<MorePanel> {
  ...
  List<MorePanelItem> itemList(TUIChatSeparateViewModel model, TUITheme theme) {
    ...
    return [
      ...
      if (!PlatformUtils().isWeb)
        MorePanelItem(
            id: "photo",
            title: TIM_getCurrentDeviceLocale().contains("zh") ? "相册" : "Album",//TIM_t("照片"),
            onTap: (c) {
              _onFeatureTap(
                "photo",
                c,
      ...
      if (isInstallCallkit && PlatformUtils().isMobile)
        MorePanelItem(
            id: "redEnvelope",
            title: TIM_getCurrentDeviceLocale().contains("zh") ? "红包" : "Envelope",
            onTap: (c) {
              _onFeatureTap(
                "redEnvelope",
                c,
  ...
  // 发送合并消息
  _sendMixedMessage(TUIChatSeparateViewModel model) async {
  ...
  _sendVideoMessage(AssetEntity asset, TUIChatSeparateViewModel model) async {
    ...
    if (size >= 104857600) {
      ...
      ToastUtils.showToast(msg: TIM_t("发送失败,视频不能大于100MB"), context: context);
  ...
  _sendImageMessage(TUIChatSeparateViewModel model, TUITheme theme) async {
    try {
      ...
      if (PlatformUtils().isMobile) {
        bool isZh = TIM_getCurrentDeviceLocale().contains("zh");
        ...
        AssetPickerResultData? pickedAssets = await AssetPicker.pickAssets(context,
            pickerConfig: AssetPickerConfig(
              ...
              pickerTheme: MediaQuery.platformBrightnessOf(context)== Brightness.dark?ThemeData.dark():ThemeData.light(),
              maxAssets: 24,
              showMergeSend: true,
        ...
        if (pickedAssets != null) {
          // TODO: 判断原图以及是否合并
          if (pickedAssets.isMinxImage) {
            final List<String> picturePaths = [];
            for (var asset in pickedAssets.assets) {
              final originFile = await asset.originFile;
              final filePath = originFile?.path;
            ...
            if (picturePaths.length > 1) {
              widget.mergePictureCallback?.call(picturePaths);
            } else if (picturePaths.length == 1) {
              MessageUtils.handleMessageError(
                  model.sendImageMessage(
                      imagePath: picturePaths.first,
                      isOriginImage: pickedAssets.isNativeImage,
                      convID: convID,
                      convType: convType),
                  context);
            }
  ...
  _goToVideoUI(String type) async {
    ...
    if (isGroup) {
      if (widget.getSelectMember != null) {
        final inviteMember = await widget.getSelectMember!(widget.conversationID,[], [], 8);
        ...
        List<V2TimGroupMemberFullInfo>? selectedMember = await Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => TimCallSelectMemberPage(
