...
class TIMUIKitInputTextField extends StatefulWidget {
  ...
  const TIMUIKitInputTextField(
      ...
      this.groupID,
      this.chatConfig,
        this.getSelectMember, this.sendRedEnvelope, this.mergePictureCallback,
      })
      : super(key: key);
...
class _InputTextFieldState extends TIMUIKitState<TIMUIKitInputTextField> {
  ...
  void deleteStickerFromText() {
    ...
    if (originalText == zeroWidthSpace) {
      ...
      print("删除按钮回调 - ${currentCursor} - ${originalText.length}");
      if (cursorPosition > 0) {
        ...
        currentCursor = (currentCursor ?? originalText.length) - removeLength;
      }
...
// index为emoji的index,data为baseurl+name
  ...
  onSubmitted() async {
    ...
    final convType = widget.conversationType;
    if (text.isNotEmpty && text != zeroWidthSpace) {
      if (widget.model.editorMessage != null) {
        print("编辑消息 - ${widget.model.editorMessage?.textElem?.text}");
        print("编辑消息1 - ${text}");
        final originMessage = widget.model.editorMessage!;
        originMessage.textElem = V2TimTextElem(
          text: text
        );
        ...
        //   return;
        // }
      } else if (widget.model.repliedMessage != null) {
  ...
  _getMuteType(TUIChatSeparateViewModel model) async {
    ...
    if (widget.conversationType == ConvType.group && !willNotBeenMuted) {
      ...
      } else if ((!(model.groupInfo?.isAllMuted ?? false))&&selfModel.loginInfo?.userID != null && await
  ...
  Widget tuiBuild(BuildContext context, TUIKitBuildValue value) {
    ...
    _getMuteType(model);
    final hasEditorMessage = model.editorMessage != null;
    return Selector<TUIChatSeparateViewModel, V2TimMessage?>(
        builder: ((context, value, child) {
          String? getForbiddenText() {
            String title = TIM_getCurrentDeviceLocale().contains("zh") ? "群组不存在" : "No Group";
            if (!(model.isGroupExist)) {
              title = TIM_getCurrentDeviceLocale().contains("zh") ? "群组不存在" : "No Group";
              return title;
            } else if (model.isNotAMember) {
              title = TIM_getCurrentDeviceLocale().contains("zh") ? "您不是群成员" : "You are not in the group";
              return title;
            } else if (muteStatus == MuteStatus.all) {
              title = TIM_getCurrentDeviceLocale().contains("zh") ? "全员禁言中" : "Forbidden Speech All";
              return title;
            } else if (muteStatus == MuteStatus.me) {
              title = TIM_getCurrentDeviceLocale().contains("zh") ? "您被禁言" : "Forbidden Speech You";
              return title;
            }
            return null;
          }
          ...
          return LayoutBuilder(builder: (BuildContext context, BoxConstraints constraints) {
            ...
            return TUIKitScreenUtils.getDeviceWidget(
                ...
                defaultWidget: TIMUIKitTextFieldLayoutNarrow(
                    ...
                    repliedMessage: hasEditorMessage ? null : value,
                    editorMessage: hasEditorMessage ? value : null,
        ...
        selector: (c, model) => (hasEditorMessage ? model.editorMessage : model.repliedMessage ));
  }
}
