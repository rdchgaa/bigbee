...
typedef MessageChooseMemberFunction = Future<List<String>> Function(
    String groupId, List<String> selectedMemberIds, List<String> excludeMemberIds, int? maxNum);
typedef MessageSendRedEnvelopeFunction = Function();
typedef MessageCollectFunction = Future<bool> Function(List<V2TimMessage> messageList);
typedef MergePictureFunction = Function(List<String>);
...
class TIMUIKitChat extends StatefulWidget {
  ...
  /// 修改导航栏【ab】24.07.12
  final PreferredSizeWidget? appBarConfig;
 ...
 // 合并图片
  ...
  TIMUIKitChat({
    ...
    this.sendRedEnvelope,
    this.collectionCallback, this.mergePictureCallback,
  }) : super(key: key) {
    startTime = DateTime.now().millisecondsSinceEpoch;
  }

  @override
  State<StatefulWidget> createState() => TUIChatState();
}

class TUIChatState extends TIMUIKitState<TIMUIKitChat> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      updateJoinInGroupCallWidget();
    });
    Future.delayed(const Duration(milliseconds: 500), () {
      updateDraft();
    });

    // final state = TUICallKit.state;
    // print("当前通话 - ${state.groupId} - ${widget.selectedConversation.groupID} - ${state.remoteUserList.length}");
  }
  ...
  updateJoinInGroupCallWidget() async {
    if (_getConvType() != ConvType.group) {
      if (_joinInGroupCallWidget != null) {
        setState(() {
          _joinInGroupCallWidget = null;
        });
      }
      return;
    }
    final w =
        await TUICore.instance.raiseExtension(TUIExtensionID.joinInGroup, {GROUP_ID: widget.conversationID ?? ""});
    if (w != _joinInGroupCallWidget) {
      print("更新joinInGroupCallWidget - ${w}");
      print("更新joinInGroupCallWidget1 - ${_joinInGroupCallWidget}");
      setState(() {
                            ...
                            child: Align(
                                ...
                                child: Listener(
                                  ...
                                  child: TIMUIKitHistoryMessageListContainer(
                                    ...
                                    conversationID: _getConvID(), messageCollectFunction: widget.collectionCallback,
                                  ),
                                )),
                          )),
                          ...
                          Selector<TUIChatSeparateViewModel, bool>(
                            builder: (context, value, child) {
                              return value
                                  ? MultiSelectPanel(
                                      conversationType: _getConvType(),
                                      collectionCallback: widget.collectionCallback,
                                    )
                                  : (widget.textFieldBuilder != null
                                      ...
                                      : TIMUIKitInputTextField(
                                          ...
                                          sendRedEnvelope: widget.sendRedEnvelope, mergePictureCallback: widget.mergePictureCallback,
                                        ));
                            },
                            selector: (c, model) {
