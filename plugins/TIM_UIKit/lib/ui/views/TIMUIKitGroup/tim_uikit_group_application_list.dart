...
typedef GroupApplicationItemBuilder1 = Widget Function(BuildContext context, V2TimGroupApplication applicationInfo, int index, ApplicationStatus status, Function(bool) onTap);
...
class TIMUIKitGroupApplicationList extends StatefulWidget {
  ...
  final GroupApplicationItemBuilder1? itemBuilder1;

  /// group ID
  final String groupID;

  const TIMUIKitGroupApplicationList({Key? key, this.itemBuilder, this.itemBuilder1, required this.groupID}) : super(key: key);
...
class TIMUIKitGroupApplicationListState extends TIMUIKitState<TIMUIKitGroupApplicationList> {
  ...
  void initState() {
    super.initState();
    groupApplicationList = model.groupApplicationList.where((item) => (item.groupID == widget.groupID && item.handleStatus == 0)).toList();
        ...
        return Container(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              if (widget.itemBuilder1 != null) {
                final ApplicationStatus currentStatus = applicationStatusList[index];
                return widget.itemBuilder1!(context, applicationInfo, index, currentStatus, (value) async {
                  if (value) {
                    final res = await _groupServices.acceptGroupApplication(
                      groupID: applicationInfo.groupID,
                      fromUser: applicationInfo.fromUser!,
