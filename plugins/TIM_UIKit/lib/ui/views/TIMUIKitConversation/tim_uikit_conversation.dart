...
class TIMUIKitConversation extends StatefulWidget {
  ...
  /// the builder for conversation item
  final ConversationItemBuilder? itemContentBuilder;
  ...
  State<StatefulWidget> createState() {
    return TIMUIKitConversationState();
  }
}
...
class TIMUIKitConversationState extends TIMUIKitState<TIMUIKitConversation> {
  ...
  List<V2TimConversation?> getFilteredConversation() {
    List<V2TimConversation?> filteredConversationList = model.conversationList.where((element) => ((element?.groupID != null || element?.userID != null) && element?.userID != "10124410")).toList();
  ...
  Widget _defaultSecondaryMenu(V2TimConversation conversationItem, VoidCallback onClose) {
    return TUIKitColumnMenu(data: [
      ...
      ColumnMenuItem(
          ...
          onClick: () {
            onClose();
            pinConversation(conversationItem);
          }),
      ColumnMenuItem(
          label: TIM_t("删除会话"),
            ...
            return filteredConversationList.isNotEmpty
                ? ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      List<ConversationItemSlidePanel> slideChildren = [];//_getSlideBuilder()(conversationItem!);
                      if (widget.itemSlideBuilder != null && widget.itemSlideBuilder!(conversationItem!) != null) {
                        slideChildren = widget.itemSlideBuilder!(conversationItem)!;
                      } else {
                        slideChildren = _defaultSlideBuilder(conversationItem!);
                      }
                      // final slideChildren = widget.itemSlideBuilder ?? _defaultSlideBuilder
                      ...
                      Widget conversationLineItem(V2TimUserStatus? onlineStatus, ConversationItemBuilder? itemContentBuilder) {
                        return Material(
                          ...
                          child: GestureDetector(
                            child: itemContentBuilder != null ?
                            itemContentBuilder(conversationItem, onlineStatus) :
                      ...
                      return TUIKitScreenUtils.getDeviceWidget(
                          ...
                          desktopWidget: AutoScrollTag(
                            ...
                            child: InkWell(
                              ...
                              child: conversationLineItem(onlineStatus, widget.itemContentBuilder),
                            ),
                          ),
                          defaultWidget: AutoScrollTag(
                            ...
                            controller: _autoScrollController,
                            index: index,
                            child: Slidable(groupTag: 'conversation-list', child: conversationLineItem(onlineStatus, widget.itemContentBuilder), endActionPane: ActionPane(extentRatio: slideChildren.length > 2 ? 0.77 : 0.5, motion: const DrawerMotion(), children: slideChildren)),
                          ));
                    })
