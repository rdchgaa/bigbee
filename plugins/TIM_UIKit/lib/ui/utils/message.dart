...
import 'custom_group_tip_message_data_provider.dart';
import 'custom_message_utils.dart';

class MessageUtils {

  // 判断是否是删除消息
  static isDeleteMessage(V2TimMessage message) {
    final cloudCustomData = message.cloudCustomData;
    if (cloudCustomData != null) {
      try {
        final Map<String, dynamic> dataMap = jsonDecode(cloudCustomData);
        final isDelete = dataMap["isDelete"];
        if (isDelete == true) {
          return true;
        }
      } catch (e) {
  ...
  static String getGroupCallText(V2TimMessage message) {
    if (!isGroupCallingMessage(message)) {
      return "";
    }
    CallingMessageDataProvider callingMessageDataProvider = CallingMessageDataProvider(message);
    if (callingMessageDataProvider.participantType == CallParticipantType.group) {
      return callingMessageDataProvider.newContent;
    }
    return "";
  }
  ...
  static bool isCustomGroupTipMessage(V2TimMessage message) {
    final isGroup = message.groupID != null;
    final isCustomMessage =
        message.elemType == MessageElemType.V2TIM_ELEM_TYPE_CUSTOM;
    if (isCustomMessage && isGroup) {
      final customElemData = message.customElem?.data ?? "";
      try {
        Map<String, dynamic> customMap = jsonDecode(customElemData);
        if (customMap.containsKey('businessID') && customMap['businessID'] == customGroupTipBusinessID) {
          return true;
        }
      } catch (e) {
        return false;
  ...
  static String getCustomGroupTipText(V2TimMessage message) {
    if (!isCustomGroupTipMessage(message)) {
      return "";
    }
    CustomGroupTipMessageDataProvider callingMessageDataProvider = CustomGroupTipMessageDataProvider(message);
  ...
  // 判断消息是否可以被收藏
  static bool isMessageCanCollect({required V2TimMessage message}) {
  ...
  // 判断消息是否可以被删除
  static bool isMessageCanDelete({required V2TimMessage message}) {
    if (CustomMessageUtils.isRedEnvelopeMessage(message)) {
      return false;
    }
    return true;
  }
  ...
  // 判断消息是否可以被转发
  static bool isMessageCanForward({required V2TimMessage message}) {
    if (message.status != MessageStatus.V2TIM_MSG_STATUS_SEND_SUCC) {
      return false;
    }
    if (CustomMessageUtils.isRedEnvelopeMessage(message) ||
    CustomMessageUtils.isCallingMessage(message)
    ) {
      return false;
    }
    return true;
