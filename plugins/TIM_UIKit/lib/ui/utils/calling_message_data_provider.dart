...
class CallingMessageDataProvider {
  ...
  // 最后一条消息描述
  String? get lastMessageDesc => _getLast();
  ...
  _setContent() {
    ...
    if (_participantType == CallParticipantType.c2c) {
      ...
      if (_protocolType == CallProtocolType.send) {
        ...
        _newContent = showName + TIM_t('发起了群通话');
      } else if (_protocolType == CallProtocolType.cancel) {
        _content = TIM_t('通话结束');
        _newContent = TIM_t('通话结束');
      } else if (_protocolType == CallProtocolType.hangup) {
        _content = TIM_t('通话结束');
        _newContent = TIM_t('通话结束');
  ...
  String? _getLast() {
    if (_isCallingSignal) {
      if (_participantType == CallParticipantType.group && _newContent.isNotEmpty) {
        return _newContent;
      } else {
        switch (_streamMediaType) {
          case CallStreamMediaType.audio:
            return "[${TIM_t("语音通话")}]";
          case CallStreamMediaType.video:
            return "[${TIM_t("视频通话")}]";
          case CallStreamMediaType.unknown:
            return TIM_t("[音视频通话]");
        }
      }
    }
