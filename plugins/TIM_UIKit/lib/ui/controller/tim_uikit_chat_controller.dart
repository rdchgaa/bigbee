...
class TIMUIKitChatController {
  ...
  // 修改消息
  Future<void> modifyMessage({required String msgID, required V2TimMessage modifiedMessage}) async {
    return model!.updateMessageFromController(msgID: msgID, message: modifiedMessage);
  }

  // 通过Id获取消息
  Future<V2TimMessage?> getMessage({required String msgID}) async {
    return model!.getMessageByID(msgID);
  }
