  ...
  Tuple<double, double> _getTopLeft(Map<int, bool> blockBigger, int index, int count) {
    ...
    if (has) {
      if (selfIsBigger) {
        ...
        j = (j > 1) ? 1 : j;
        return Tuple(
            MediaQuery.of(context).size.width * i / 4, MediaQuery.of(context).size.width * j / 4);
      }
      ...
      for (int i = 0; i < GroupCallUserWidgetData.canPlaceSquare.length; i++) {
        for (int j = 0; j < GroupCallUserWidgetData.canPlaceSquare[i].length; j++) {
          if (GroupCallUserWidgetData.canPlaceSquare[i][j] == true) {
            ...
            return Tuple(MediaQuery.of(context).size.width * i / 4,
                MediaQuery.of(context).size.width * j / 4);
          }
        }
      }
    }
    ...
    if (count == 2) {
      if (index == 1) {
        return Tuple(MediaQuery.of(context).size.width / 4, 0);
      }
      return Tuple(MediaQuery.of(context).size.width / 4, MediaQuery.of(context).size.width / 2);
    }
    if (count == 4) {
      if (index == 1) {
        return Tuple(0, 0);
