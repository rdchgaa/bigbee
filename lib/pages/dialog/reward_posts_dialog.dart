...
class DialogRewardPostBox extends StatefulWidget {
  final int postId;

  const DialogRewardPostBox({
    Key? key,
    required this.postId,
  }) : super(key: key);

  @override
  _DialogRewardPostBoxState createState() => _DialogRewardPostBoxState();
}

class _DialogRewardPostBoxState extends State<DialogRewardPostBox> {
  ...
  initData() async {
    var resultRecords = await DynamicsNet.dynamicsRewardOptions();
  ...
  submit() async {
    ...
    if (selectedReward?.type == 1) {
      ...
      hotTimes = inputTime;
      payNum =inputTime.toDouble();
    }
    final result = await DynamicsNet.dynamicsRewardPosts(
      postId: widget.postId,
      payPrice: payNum??0, rewardSettingId: selectedReward?.id??0,
    );
    if (result.code == 200) {
      Navigator.of(context).pop(true);
      ABToast.show(AB_S().rewardSuccessful,toastType: ToastType.success);
    } else {
                    ...
                    builder: (context) {
                      ...
                      return Padding(
                        ...
                        child: ABButton.gradientColorButton(
                          ...
                          cornerRadius: 12,
                          height: 48,
                          text: AB_getS(context).rewardImmediately,
                          onPressed: () {
                            if(canClick) submit();
                          },
                        ),
  ...
  Widget build(BuildContext context) {
    ...
    return Center(
      child: TextField(
        ...
        inputFormatters: [LengthLimitingTextInputFormatter(8), FilteringTextInputFormatter.allow(RegExp("[0-9]"))],
      ),
    );
  }
}
