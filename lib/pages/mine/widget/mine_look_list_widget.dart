...
class MineLookListWidget extends StatefulWidget {
  final MineLookListType type;
  const MineLookListWidget({super.key, required this.type});

  @override
  State<MineLookListWidget> createState() => _MineLookListWidgetState();
}

class _MineLookListWidgetState extends State<MineLookListWidget> {
  late MineLookListVm _vm;

  @override
  void initState() {
    _vm = MineLookListVm(type: widget.type);
  ...
  Widget build(BuildContext context) {
    ...
    return ChangeNotifierProvider(
      ...
      builder: (context, child) {
        var vm = context.watch<MineLookListVm>();
        return Column(
        ...
        child: Row(
          ...
          children: [
                ...
                Row(
                  ...
                  children: [
                    Container(
                      width: 10.px,
                      height: 10.px,
                      decoration: BoxDecoration(color: isOnline ? Colors.green : theme.textGrey, borderRadius: BorderRadius.circular(5.px)),
                    ),
                    SizedBox(width: 6.px,),
            ...
            _getFollowStatusWidget(focusStatus: model.isFocus ?? 0, onPressed: () async {
              if (!isFollowed) {
                await _vm.changeFollowStatus(model.memberNum ?? "",oldFollow: model.isFocus);
                return;
              }
              if (!mounted) {
                return;
              ...
              if (isConfirmed == true) {
                final res = await _vm.changeFollowStatus(model.memberNum ?? "",oldFollow: model.isFocus);
                if (res) {
  ...
  Widget _getFollowStatusWidget({int focusStatus = 0, VoidCallback? onPressed}) {
    ...
    if (focusStatus == 0 ||focusStatus == 2) {
