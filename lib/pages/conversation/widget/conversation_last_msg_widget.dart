...
class ConversationLastMsgWidget extends StatefulWidget {
  final V2TimMessage? lastMsg;
  final List<V2TimGroupAtInfo?> groupAtInfoList;
  final BuildContext context;
  final double fontSize;

  const ConversationLastMsgWidget({Key? key, this.lastMsg, required this.groupAtInfoList, required this.context, this.fontSize = 14.0}) : super(key: key);

  @override
  State<ConversationLastMsgWidget> createState() => _TIMUIKitLastMsgState();
}

class _TIMUIKitLastMsgState extends TIMUIKitState<ConversationLastMsgWidget> {
  ...
  @override
  void didUpdateWidget(covariant ConversationLastMsgWidget oldWidget) {
  ...
  Future<String?> _getLastMsgShowText(V2TimMessage? message, BuildContext context) async {
    if (message?.sender == systemUserId) {
      return AB_getS(context).notice;
    }
