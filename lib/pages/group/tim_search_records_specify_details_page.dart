...
class TimSearchRecordsSpecifyDetailsPage extends StatefulWidget {
  ...
  const TimSearchRecordsSpecifyDetailsPage(
      {Key? key,
      this.isAutoFocus = true,
      required this.currentConversation,
      required this.onTapConversation,
      this.initMessageList,
      this.searchTimePosition, this.userId, this.title})
      : super(key: key);

  @override
  State<StatefulWidget> createState() => TimSearchRecordsSpecifyDetailsPageState();
}

class TimSearchRecordsSpecifyDetailsPageState extends TIMUIKitState<TimSearchRecordsSpecifyDetailsPage> {
  ...
  updateMsgResult(bool isNewSearch) async{
    ...
    await model.getMsgForDate(
      widget.currentConversation.conversationID,
      currentPage,
      searchTimePeriod: widget.searchTimePosition==null?0:24 * 60 * 60,
      searchTimePosition: widget.searchTimePosition,
      userIDList: widget.userId==null?[]:[widget.userId!],
    );
                  ...
                  child: (currentMsgListForConversation.length == 0)
                      ...
                      : ListView(
                          children: [
                            ...
                            // _renderShowALl(totalMsgInConversationCount > currentMsgListForConversation.length)
                          ],
                        ),
                )),
              ],
